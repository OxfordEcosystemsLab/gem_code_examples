...
data_all<-read.table("SAFE_IngrowthCores_data_toR.txt", header=T, )
  # filter(Plot=="BSouth")%>%
  # filter(Date.value==41037)%>%
  # filter(Core==1)
names(data_all)
dim(data_all)

### TO investigate the strange problem between versions of codes
## BSouth.41037.1  VS   BSouth_1_2012_05_08_N
#I have to make some pserudo data as below

census<-data_all%>%
  mutate(is_stock=ifelse(DataType=='NPP', 'N','Y'))%>%
  mutate(Date_value=as.Date(Date.value, origin = "1899-12-30"))%>%
  mutate(time_step_minutes=Search.time.cum/Search.Int)%>%
  filter(Plot=="BSouth")%>%
  select(TotRoot.cum,Plot,Date_value,is_stock,DaysBetween,Search.Int,Core,time_step_minutes)%>%
  rename(
    my_data=TotRoot.cum,
    ingrowth_core_num=Core,
    plot_code=Plot,
    time_step=Search.Int
  )%>%
  mutate(
    
   year= format(Date_value,'%Y'),
    day=format(Date_value,'%d'),
...
colMeans(ExponentialRise, na.rm=T)
...
write.table(IC.ByCore, "SAFE_IC_fromR_byCore.csv", row.names=F)
write.csv(IC.ByCore, file=paste0("SAFE_IC_fromR_byCore.csv"))
...
write.csv(IC.PlotMeans.SE, file=paste0("SAFE_IC_fromR2.csv"))

                          ...
                          Carbon = FALSE, Dlim = NULL, plot = NULL, Carbon_sd=2.06) 
{
  ...
  if (!is.null(Dpropag)) {
    if (length(Dpropag) == 1 && tolower(Dpropag) == "chave2004") {
      ...
      chaveError <- function(x, len) {
        ...
        x <- myrtruncnorm(n = len, mean = x, sd = D_sd, 
                          lower = 0.1, upper = 500)
        return(x)
      }
      D_simu <- suppressWarnings(replicate(n, chaveError(D, 
                                                         len)))
    }
    else {
  ...
  WD_simu <- suppressWarnings(replicate(n, myrtruncnorm(n = len, 
                                                        mean = WD, sd = errWD, lower = 0.08, upper = 1.39)))
  if (!is.null(HDmodel) | !is.null(H)) {
    if (!is.null(HDmodel)) {
      H_simu <- apply(D_simu, 2, function(x) predictHeight(x, 
  ...
  if (!is.null(coord)) {
    ...
    Esim <- tcrossprod(as.matrix(param_7[selec, c("temp", 
                                                  "prec", "cwd")]), as.matrix(bioclimParams))
    AGB_simu <- t(t(log(WD_simu)) * param_7[selec, "logwsg"] + 
                    t(log(D_simu)) * param_7[selec, "logdbh"] + t(log(D_simu)^2) * 
                    param_7[selec, "logdbh2"] + Esim * -param_7[selec, 
                                                                "E"] + param_7[selec, "intercept"])
    matRSE <- mapply(function(y) {
      rnorm(sd = y, n = len)
    }, y = RSE)
  ...
  if (Carbon == FALSE) {
    ...
    res <- list(meanAGB = mean(sum_AGB_simu), medAGB = median(sum_AGB_simu), 
                sdAGB = sd(sum_AGB_simu), credibilityAGB = quantile(sum_AGB_simu, 
                                                                    probs = c(0.025, 0.975)), AGB_simu = AGB_simu)
  }else{
    AGC_simu <- AGB_simu * rnorm(mean = 47.13, sd = Carbon_sd, 
                                 n = n * len)/100
    sum_AGC_simu <- colSums(AGC_simu, na.rm = TRUE)
    res <- list(meanAGC = mean(sum_AGC_simu), medAGC = median(sum_AGC_simu), 
                sdAGC = sd(sum_AGC_simu), credibilityAGC = quantile(sum_AGC_simu, 
                                                                    probs = c(0.025, 0.975)), AGC_simu = AGC_simu,RSE=RSE,WD_simu=WD_simu,H_simu=H_simu,AGB_simu=AGB_simu)
  }
  return(res)
}
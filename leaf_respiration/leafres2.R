...
## open Boldata:
library(R.matlab)
setwd("F:/Side_project/african_data_workshop/General/Dataset examples/leaf_respiration/Original-matlab-code/Ken_Iq-Tang")
...
# wet leaf area?? no, it should be assimilation rate (photosynthesis) in wet season
...
# dry leaf area??no, it should be assimilation rate (photosynthesis) in dry season
...
# get water contents with plott==plotname % simply a filter can do the job
...
# ind is used to locate year and month, same as group by (year,month)
...
for (j in (fir_year:fir_yeare)) {
    ...
    totlaistdA[m,n] = mean(laistdA[ind],na.rm=T); #All these mean is making no sense! ind is a single value!
...
# Huanyuan: I can't understand what they are trying to do here:
# I think they want to bring the month to month variation of aridity into leaves respiration?
